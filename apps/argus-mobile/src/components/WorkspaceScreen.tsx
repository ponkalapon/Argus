...
const createChatTitle = (content: string) => {
  ...
  return oneLine.length > 36 ? `${oneLine.slice(0, 36).trim()}…` : oneLine || 'New chat';
};
...
export const WorkspaceScreen = ({ settings, apiKey, onOpenSettings, onOpenSandbox, onOpenFiles, onSaveSettings, pendingAttach, onClearPendingAttach }: Props) => {
  ...
  const modelLabel = settings.model.trim() || t('workspace.selectModel');
  ...
  useEffect(() => {
      ...
      if (baseUrl.startsWith('http://') && !baseUrl.startsWith('https://')) {
        setModelLoadError(t('workspace.httpBlocked'));
      ...
      fetch(`${baseUrl}/v1/models`, {
        ...
        .then(async (r) => {
          ...
          if (Array.isArray(json)) {
            ...
            throw new Error(t('workspace.unknownResponse'));
          }
          if (cancelled) return;
          if (modelIds.length === 0) {
            setModelLoadError(t('workspace.emptyModelList'));
          }
        ...
        .catch((err) => {
          clearTimeout(timeout);
          if (cancelled) return;
          const message = err instanceof Error ? err.message : t('workspace.loadModelsFailed');
  ...
  const startStreamTicker = (messageId: string) => {
    ...
    streamTimer.current = setInterval(() => {
      ...
      const queueLen = streamQueue.current.length;
      let nextChunkSize: number;
      if (queueLen < 8) {
        nextChunkSize = Math.min(queueLen, 1);
      } else if (queueLen < 50) {
        nextChunkSize = Math.min(Math.ceil(queueLen / 4), 6);
      } else {
        nextChunkSize = Math.min(Math.max(Math.ceil(queueLen / 8), 4), 32);
      }
  ...
  const handleCamera = async () => {
    ...
    if (!perm.granted) {
      Alert.alert(t('workspace.cameraAccessTitle'), t('workspace.cameraAccessMessage'));
      return;
    }
  ...
  const handlePhotoLibrary = async () => {
    ...
    if (!perm.granted) {
      Alert.alert(t('workspace.cameraAccessTitle'), t('workspace.galleryAccessMessage'));
      return;
    }
  ...
  const handleAttachDocument = async () => {
    try {
      ...
      if (message === PDF_UNSUPPORTED_MESSAGE) {
        Alert.alert(
          t('workspace.pdfUnsupported'),
          t('workspace.pdfUnsupportedHint'),
        );
        return;
      }

      Alert.alert(t('workspace.cameraAccessTitle'), t('workspace.fileLoadError'));
    }
  };
  ...
  const handleVoiceToggle = async () => {
    if (isRecording) {
      ...
      if (granted !== PermissionsAndroid.RESULTS.GRANTED) {
        Alert.alert(t('workspace.cameraAccessTitle'), t('workspace.microphoneAccessMessage'));
        return;
      }
      setIsRecording(true);
      try {
        ...
        setIsRecording(false);
        Alert.alert(t('workspace.cameraAccessTitle'), t('workspace.recordStartError'));
      }
    }
  };
  ...
  const requestContactDisclosure = useCallback((payload: { query: string; results: ContactSafePreview[] }) => {
    ...
    return new Promise<boolean>((resolve) => {
      Alert.alert(
        t('workspace.confirmPhoneTitle'),
        t('workspace.confirmPhoneMessage', { query: payload.query }),
        [
          { text: t('workspace.denyPhone'), style: 'cancel', onPress: () => resolve(false) },
          { text: t('workspace.allowPhone'), onPress: () => resolve(true) },
        ],
      );
    });
  }, []);
  ...
  const confirmCommunication = useCallback((payload: { action: 'call' | 'sms'; phone: string; name?: string }) => {
    ...
    const actionLabel = payload.action === 'call' ? t('workspace.callAction') : t('workspace.smsAction');

    return new Promise<boolean>((resolve) => {
      Alert.alert(
        t('workspace.confirmActionTitle'),
        t('workspace.confirmActionMessage', { name: payload.name || '—', phone: payload.phone, action: payload.action }),
        [
          { text: t('workspace.cancelAction'), style: 'cancel', onPress: () => resolve(false) },
          {
            text: actionLabel,
            onPress: () => {
  ...
  const handleContactsSearch = async () => {
    if (!settings.allowAssistantContacts) {
      Alert.alert(t('workspace.contactsDisabledTitle'), t('workspace.contactsDisabledHint2'), [
        { text: t('workspace.cancelAction'), style: 'cancel' },
        { text: t('workspace.settingsButton'), onPress: onOpenSettings },
      ]);
      return;
    }
    ...
    try {
      ...
      if (results.length === 0) {
        Alert.alert(t('workspace.contactsNotFoundTitle'));
        return;
      }
      const safeResults = results.map(toSafeContactPreview);
      const names = safeResults.map((c) => `${c.name} — ${c.maskedPhones.join(', ')}`).join('\n');
      setDraft(t('workspace.findContactPrompt', { query: names }));
    } catch {
      Alert.alert(t('workspace.cameraAccessTitle'), t('workspace.contactsLoadError'));
    }
  };
  ...
  const sendMessage = async (text: string) => {
    ...
    if (!hasRequiredSettings) {
      Alert.alert(t('workspace.settingsRequired'), t('workspace.settingsRequiredHint'), [
        { text: t('workspace.cancelAction'), style: 'cancel' },
        { text: t('workspace.openSettingsButton'), onPress: onOpenSettings },
      ]);
      return;
    }
    ...
    const augmentedContent = context 
      ? t('workspace.contextPrefix', { context, content })
      : content;
    ...
    try {
      ...
      if (result.usage) {
        setInputTokens(result.usage.input);
        setOutputTokens(result.usage.output);
      ...
      streamMessageId.current = null;
      let message =
        requestError instanceof Error ? requestError.message : t('workspace.unknownRequestError');
      ...
      if (attachedDocs.some(d => d.name?.match(/\.(png|jpg|jpeg|gif|webp|bmp|heic|heif)$/i))) {
        ...
        if (lower.includes('image') || lower.includes('vision') || lower.includes('picture') || lower.includes('photo')) {
          message = t('workspace.imageNotSupported');
        }
      }
      ...
      const errorMessages = visibleMessages.map((item) =>
        item.id === assistantMessage.id
          ? { ...item, content: item.content.trim() ? item.content : `${t('workspace.errorPrefix')} ${message}` }
          : item,
      );
  ...
  const openChat = async (chat: StoredChat) => {
    ...
    setMessages(chat.messages.length ? chat.messages : [createInitialMessage()]);
    setAttachedDocs([]);
    setError('');
    setStatus('idle');
  ...
  const deleteChat = (chatId: string) => {
    Alert.alert(t('workspace.deleteChatTitle'), t('workspace.deleteChatMessage'), [
      { text: t('workspace.cancelAction'), style: 'cancel' },
      {
    ...
    <View style={styles.slideRoot}>
            ...
            >
              {isEmptyChat ? (
                <View style={styles.welcomeWrap}>
                  <Text style={styles.welcomeTitle}>{t('workspace.welcomeTitle')}</Text>
                  {!hasRequiredSettings && (
                    <Pressable
                    ...
                    >
                      ...
                      <Text style={styles.connectHintText}>{t('workspace.connectHint')}</Text>
                    </Pressable>
                  )}
                </View>
              ) : (
              ...
              <View style={styles.statusBar}>
                <View style={styles.statusLeft}>
                  ...
                  <Text style={styles.tokenText}>
                    {t('workspace.tokenInput')} {formatTokenNumber(animInputTokens)}
                    {' / '}
                    {t('workspace.tokenOutput')} {formatTokenNumber(animOutputTokens)}
                  </Text>
                </View>
        ...
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <ScrollView style={styles.navScroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            {isSearching ? (
              <>
                {searchQuery.trim() === '' ? (
                  <View style={styles.emptyChatsBox}>
                    <Text style={styles.emptyChatsTitle}>{t('workspace.enterQuery')}</Text>
                    <Text style={styles.emptyChatsText}>{t('workspace.searchHint')}</Text>
                  </View>
                ) : searchResults.length === 0 ? (
                  <View style={styles.emptyChatsBox}>
                    <Text style={styles.emptyChatsTitle}>{t('workspace.nothingFound')}</Text>
                    <Text style={styles.emptyChatsText}>{t('workspace.tryChangeQuery')}</Text>
                  </View>
                ) : (
                  <>
                    <Text style={[styles.navSectionHeader, { marginTop: spacing.sm }]}>{t('workspace.searchResults')}</Text>
                    {searchResults.map((result) => (
                      ...
                      >
                        <View style={styles.chatItemTextWrap}>
                          ...
                          <Text style={styles.chatItemMeta}>{formatChatDate(result.updatedAt)} · {t('workspace.match')} {Math.round(result.score * 100)}%</Text>
                        </View>
                      </Pressable>
                    ))}
                  </>
              ...
              <>
                {/* Навигация */}
                <Text style={[styles.navSectionHeader, { marginTop: spacing.sm }]}>{t('workspace.navigation')}</Text>
                <View style={styles.navCol}>
                  ...
                  >
                    ...
                    <Text style={styles.navRowLabel}>{t('workspace.newChat')}</Text>
                  </Pressable>
                  <Pressable
                    onPress={() => openSearch()}
                    style={({ pressed }) => [styles.navRowItem, pressed && styles.pressed]}
                  >
                    ...
                    <Text style={styles.navRowLabel}>{t('workspace.search')}</Text>
                  </Pressable>
                  <Pressable
                    onPress={async () => {
                      if (activeChatId) {
                  ...
                  >
                    ...
                    <Text style={styles.navRowLabel}>{t('workspace.library')}</Text>
                  </Pressable>
                  <Pressable
                  ...
                  >
                    ...
                    <Text style={styles.navRowLabel}>{t('workspace.sandbox')}</Text>
                  </Pressable>
                </View>

                {/* Недавние */}
                <Text style={styles.navSectionHeader}>{t('workspace.recent')}</Text>

                {chats.length === 0 ? (
                  <View style={styles.emptyChatsBox}>
                    <Text style={styles.emptyChatsTitle}>{t('workspace.noChatsYet')}</Text>
                    <Text style={styles.emptyChatsText}>{t('workspace.firstChatHint')}</Text>
                  </View>
                ) : (
                  chats.map((chat) => {
                      ...
                      >
                        <View style={styles.chatItemTextWrap}>
                          ...
                          <Text style={styles.chatItemMeta} numberOfLines={1}>
                            {formatChatDate(chat.updatedAt)} · {t('workspace.messagesCount', { count: chat.messages.length })}
                          </Text>
                        </View>
                        <Pressable
                          accessibilityRole="button"
          ...
          <View style={styles.drawerBottom}>
            ...
            <View style={styles.drawerBottomStatus}>
              ...
              <Text style={styles.drawerBottomStatusText}>
                {hasRequiredSettings ? t('workspace.connected') : t('workspace.notConfigured')}
              </Text>
            </View>
          </View>
        </SafeAreaView>
      ...
      <Animated.View style={[styles.wsPanelContainer, { transform: [{ translateX: wsTranslate }] }]}>
        <SafeAreaView style={{ flex: 1 }}>
          <View style={styles.chatPanelHeader}>
            <Text style={styles.chatPanelTitle}>{t('workspace.workspaceFiles')}</Text>
            <Pressable
              accessibilityRole="button"
              onPress={() => {
                setPreviewFile(null);
          ...
          {!activeChatId ? (
            <View style={styles.emptyChatsBox}>
              <Text style={styles.emptyChatsTitle}>{t('workspace.noActiveChat')}</Text>
              <Text style={styles.emptyChatsText}>{t('workspace.createChatHint')}</Text>
            </View>
          ) : (
            <>
              <Pressable
              ...
              >
                ...
                <Text style={styles.newChatText}>{t('workspace.exportArchive')}</Text>
              </Pressable>

              {wsFiles.length === 0 ? (
                <View style={styles.emptyChatsBox}>
                  <Text style={styles.emptyChatsTitle}>{t('workspace.workspaceEmpty')}</Text>
                  <Text style={styles.emptyChatsText}>{t('workspace.saveFileHint')}</Text>
                </View>
              ) : (
        ...
        <>
          ...
          >
            <Pressable style={styles.modelPanel} onPress={(e) => e.stopPropagation()}>
              <Text style={styles.modelPanelTitle}>{t('workspace.selectModel')}</Text>
              ...
              <View style={styles.modelSearchWrap}>
                ...
                <TextInput
                  ...
                  placeholder={t('workspace.searchModel')}
                  placeholderTextColor={colors.textDim}
                ...
                {isLoadingModels ? (
                  <View style={styles.modelEmpty}>
                    <Animated.View style={{ opacity: modelPickerAnim }}>
                      <Text style={styles.modelEmptyText}>{t('workspace.loadingModels')}</Text>
                    </Animated.View>
                  </View>
                ) : (
                  <>
                    ...
                    {modelSearch.trim().length > 0 && (
                      <View>
                        <Text style={styles.modelProviderLabel}>{t('workspace.providerOther')}</Text>
                        <Pressable
                          onPress={() => {
                            setShowModelPicker(false);
                    ...
                    {filteredGroups.length === 0 && modelSearch.trim().length === 0 && (
                      <View style={styles.modelEmpty}>
                        {modelLoadError ? (
                          <>
                            <Text style={[styles.modelEmptyText, { color: colors.danger }]}>{t('workspace.loadError')}</Text>
                          ...
                          <Text style={styles.modelEmptyText}>{t('workspace.noModels')}</Text>
                        )}
                      </View>
                    )}
                  </>
      ...
      >
        <View style={{ paddingHorizontal: spacing.lg, gap: spacing.sm, paddingTop: spacing.md }}>
          {[
            { label: t('workspace.camera'), icon: Camera, onPress: handleCamera },
            { label: t('workspace.photo'), icon: Image, onPress: handlePhotoLibrary },
            { label: t('workspace.files'), icon: Paperclip, onPress: handleAttachDocument },
            { label: t('workspace.fileManager'), icon: Folder, onPress: handleOpenFiles },
            { label: t('workspace.contacts'), icon: Users, onPress: handleContactsSearch },
            { label: internetEnabled ? t('workspace.internetOn') : t('workspace.internetOff'), icon: Globe, onPress: () => {
              setInternetEnabled((p) => {
                const next = !p;
                void onSaveSettings({ ...settings, internetEnabled: next }, apiKey);
                return next;
              });
