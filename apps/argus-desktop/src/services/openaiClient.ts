...
const buildSystemPrompt = async (userMessage: string, context?: ChatCompletionContext) => {
  ...
  finalPrompt += '\n\n## ДОСТУП К ФАЙЛАМ И ДИСКАМ ПК\nТы автономный ИИ-агент с ПРЯМЫМ доступом к файловой системе ПК пользователя! Когда пользователь просит посмотреть папку или файл на компьютере (например H:\\Загрузки, C:\\Users, Рабочий стол и т.д.), ты ОБЯЗАН вызвать инструмент list_system_dir или read_file для просмотра содержимого диска. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО говорить, что у тебя нет доступа к дискам или файлам.';
