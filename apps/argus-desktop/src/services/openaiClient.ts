...
const buildSystemPrompt = async (userMessage: string, context?: ChatCompletionContext) => {
  ...
  finalPrompt += '\n\n## ДОСТУП И УПРАВЛЕНИЕ ФАЙЛАМИ ПК\nТы полноценный ИИ-агент с ПРЯМЫМ доступом к файловой системе ПК! Ты умеешь НЕ ТОЛЬКО читать, но и СОЗДАВАТЬ, РЕДАКТИРОВАТЬ (write_file), УДАЛЯТЬ (delete_system_file), ПЕРЕМЕЩАТЬ И СОРТИРОВАТЬ (move_system_file) файлы и папки в любых указанных местах на ПК (например H:\\Загрузки, C:\\...).\nКогда пользователь просит навести порядок, отсортировать файлы, удалить лишнее или отредактировать документы по пути — всегда вызывай соответствующие инструменты (`write_file`, `delete_system_file`, `move_system_file`, `list_system_dir`). КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО говорить, что у тебя нет доступа!';
  ...
  finalPrompt += '\n\n## ФОРМАТИРОВАНИЕ И РАЗМЫШЛЕНИЯ\nКогда тебе необходимо обдумать сложную задачу или вызов инструментов, используй блок `<thinking>...</thinking>`. Твой конечный ответ вне блока мышления должен содержать ТОЛЬКО чистый, вежливый и грамотный текст на русском языке. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать мета-префиксы вроде `[Ответ отправлен пользователю]:`, сырые вызовы JSON или отладочный мусор.';
...
export const requestChatCompletion = async ({
  ...
  const extendedCtx: ChatCompletionContext = {
    ...
    workspaceContext: workspaceCtx
      ? `Текущая рабочая область (workspace):\n${workspaceCtx}\n\nИНСТРУКЦИЯ: Ты ИИ-агент разработчик Argus. Используй инструменты работы с файлами ТОЛЬКО когда пользователь просит прочитать, создать или отредактировать код/файл. На обычные приветствия или общие вопросы отвечай вежливо и сразу простым текстом.`
      : undefined,
  };
